...
read indeks
echo -e "\e[42mpodaj nazwe uzytkownika dla tej wirtualnej maszyny (np. pierwszy / drugi ale juz bez numeru indeksu!):\e[0m"
read uzytkownik
...
echo "nadawanie uprawnien sudo dla uzytkownika $uzytkownik-$indeks"
cd /etc
sed -i "/root\tALL=(ALL:ALL) ALL/a $uzytkownik-$indeks\tALL=(ALL:ALL) ALL" /etc/sudoers
...
echo -e "\n"
echo -e "aby sprawdzic jaki jest hostname, wpisz w konsoli: hostname (wymagany restart maszyny aby zobaczyc efekt zmiany hostname)"
echo -e "\n"
echo -e "aby sprawdzic w jakich grupach jest dany user, wpisz: groups nazwa_uzytkownika"
echo -e "\n"
echo -e "aby sprawdzic aktualna date wpisze w konsoli 'date'"
echo -e "\n"
...
echo -e "\n"
echo -e "\e[42maby sprawdzic czy dziala apache, wejdz na adres IP komputera ktory posiada serwer apache (oczywiscie wpisz adres IP w przegladarce)\e[0m"